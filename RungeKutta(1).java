...
public class RungeKutta {
	...
	public static double[] solver (ODESystem system, double time) throws IllegalTimeStepException
	{
		...
		k1=k1(system, time);
		k2=k2andk3(system, time, k1);
		k3=k2andk3(system, time, k2);
		k4=k4(system, time, k3);
		for (int i=0; i<systemSize; i++)
		{
			values[i]=initialConditions[i]+0.05*(k1[i]+2*k2[i]+2*k3[i]+k4[i])/6;
		}
	return values;
	}
	...
	private static double[] k2andk3 (ODESystem system, double time, double[] k) //this method will work for both the k2 and k3 functions provided array k passed is full of k-1 values; i.e., if you're using this to find values for k2, you must supply an array of k1 values; and if you want k3 values, pass k2 values
	{
		...
		{
			currentValues[i] = currentValues[i]+0.05*k[i]/2; //replacing current values with their new Runge-Kutta values
		}
		return (system.getFunction(time+0.05/2, currentValues));
	}
	
	private static double[] k4 (ODESystem system, double time, double[] k3)//this works only for k4; the passed array MUST contain your k3 values!
	{
		double[] currentValues = new double [system.getSystemSize()];
		currentValues = system.getCurrentValues();
		for (int i = 0; i<system.getSystemSize(); i++)
		{
			currentValues[i] = currentValues[i]+0.05*k3[i]; //replacing current values with their new values
		}
		return (system.getFunction(time+0.05, currentValues));
	}
